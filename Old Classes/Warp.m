classdef Warp 
    %WARP Summary of this class goes here
    ...
    methods (Static)
        function obj = warp(obj)
            ...
            % Itakura 1975) is 'warpFactor' in this version (determined by variable 'warpFactor' the statement 
            %'if k(i-1,j)+1 >= warpFactor'). The function also allows vertical jumps of 3 rather than 2. The function
            ...
            m = length(obj.refContour); %the number of data points in the reference contour
            n = length(obj.inputContour); %the number of data points in the comparison contour
...
%             % plot(u2(warpfun), 'r');
%             drawnow
            D = N(m, n)/length(obj.warpfun);
            obj.WARP = {D, obj.warpfun};
            clear N M p;
        end

         function obj = unwarp(obj)
            for i = 1:obj.warpfun(end)
               n = find(obj.warpfun ==i);
               if isempty(n)
                  obj.unwarpfun(i) = obj.unwarpfun(i-1);
               else
               obj.unwarpfun(i) = n(1);
               end
            end
